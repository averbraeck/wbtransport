 ...
 */
public class EMAFederationManager
{
     ...
     */
    public EMAFederationManager(final String federationName, final int fmPort, final int fsPort, final String localSk3)
            throws Sim0MQException
    {
        this.fmContext = ZMQ.context(1);
        this.modelSocket = this.fmContext.socket(ZMQ.REQ); // TODO: MAYBE ROUTER
        this.modelSocket.setIdentity(UUID.randomUUID().toString().getBytes());
        this.fsSocket = this.fmContext.socket(ZMQ.REQ); // TODO: MAYBE ROUTER
    ...
    private void startModel(final String federationName, final int fsPort, final String localSk3) throws Sim0MQException
    {
        ...
        if (localSk3.equals("sk-3"))
        {
            fm1Message = SimulationMessage.encode(federationName, "FM", "FS", "FM.1", ++this.messageCount, MessageStatus.NEW,
                    "BGD1.1", "java8+", "-jar", "/home/alexandv/sim0mq/BGD/bgd.jar", "BGD1.1 %PORT% E:/BGD/",
                    "/home/alexandv/sim0mq/BGD", "", "/home/alexandv/sim0mq/BGD/out.txt", "/home/alexandv/sim0mq/BGD/err.txt",
                    false, false, false);
        ...
        {
            fm1Message = SimulationMessage.encode(federationName, "FM", "FS", "FM.1", ++this.messageCount, MessageStatus.NEW,
                    "BGD1.1", "java8+", "-jar", "E:/BGD/bgd.jar", "BGD1.1 %PORT% E:/BGD/", "E:/BGD", "",
                    "E:/BGD/out.txt", "E:/BGD/err.txt", false, false, false);
        ...
        if (replyMessage[4].toString().equals("FS.2") && replyMessage[9].toString().equals("started")
                && replyMessage[8].toString().equals("BGD1.1"))
        {
    ...
    {
        byte[] fm2Message;
        fm2Message = SimulationMessage.encode(federationName, "FM", "BGD1.1", "FM.2", ++this.messageCount, MessageStatus.NEW,
                30.0 * 3600.0 * 24.0, 10.0 * 3600.0 * 24.0, 0.0, Double.POSITIVE_INFINITY, 1, 0);
    ...
    private void setParameters(final String federationName) throws Sim0MQException
    {
        ...
        parameters.put("Flood_area", "2014-0290");
        parameters.put("Flood_duration", new Integer(30));

        for (String product : new String[] { "Textile", "Garment", "Steel", "Brick", "Food" })
        {
            for (String param : new String[] { "production", "consumption", "export", "import" })
            {
                parameters.put(product + "_" + param, new Double(1.0));
            }
        }

        for (String product : new String[] { "Textile", "Garment", "Steel", "Brick", "Food" })
        {
            parameters.put(product + "_road", new Double(1.0));
            parameters.put(product + "_rail", new Double(0.0));
            parameters.put(product + "_water", new Double(0.0));
        }

        parameters.put("Damage_road_n", new Double(0.1));
        parameters.put("Damage_road_r", new Double(0.3));
        parameters.put("Damage_road_z", new Double(0.5));
        parameters.put("Damage_ferry", new Double(0.4));
        parameters.put("Damage_bridge_a", new Double(0.1));
        parameters.put("Damage_bridge_b", new Double(0.3));
        parameters.put("Damage_bridge_c", new Double(0.5));
        parameters.put("Damage_bridge_d", new Double(0.7));
        parameters.put("Damage_waterway_1", new Double(0.1));
        parameters.put("Damage_waterway_2", new Double(0.2));
        parameters.put("Damage_waterway_3", new Double(0.4));
        parameters.put("Damage_waterway_4", new Double(0.6));
        parameters.put("Damage_ports", new Double(0.2));
        parameters.put("Damage_terminals", new Double(0.2));
        parameters.put("Damage_railways", new Double(0.0));
        parameters.put("Damage_railstations", new Double(0.0));

        parameters.put("Road_cost", new Double(6.25));
        parameters.put("Water_cost", new Double(2.19));
        parameters.put("Trs_cost", new Double(300));
        ...
        for (String parameterName : parameters.keySet())
        {
            ...
            {
                byte[] fm3Message;
                fm3Message = SimulationMessage.encode(federationName, "FM", "BGD1.1", "FM.3", ++this.messageCount,
    ...
    {
        byte[] fm4Message;
        fm4Message = SimulationMessage.encode(federationName, "FM", "BGD1.1", "FM.4", ++this.messageCount, MessageStatus.NEW);
    ...
    private void waitForSimEnded(final String federationName) throws Sim0MQException
    {
        while (!this.state.isSimulatorEnded() && !this.state.isError())
        {
            byte[] fm5Message;
            fm5Message =
                    SimulationMessage.encode(federationName, "FM", "BGD1.1", "FM.5", ++this.messageCount, MessageStatus.NEW);
    ...
    private void requestStatistics(final String federationName) throws Sim0MQException
    {
        ...
        for (String product : new String[] { "Textile", "Garment", "Steel", "Brick", "Food" })
        {
            for (String param : new String[] { "TransportCost", "TravelTime", "UnsatisfiedDemand" })
            {
                stats.add(product + "_" + param);
            }
        }
        for (String statName : stats)
        {
            ...
            {
                byte[] fm6Message;
                fm6Message = SimulationMessage.encode(federationName, "FM", "BGD1.1", "FM.6", ++this.messageCount,
                ...
                {
                    if (replyMessage[8].toString().equals(statName))
                    {
                        System.out.println("Received statistic for " + statName + " = " + replyMessage[9].toString());
                    }
                    else
                    {
                        this.state = ModelState.ERROR;
    ...
    private void killFederate(final String federationName) throws Sim0MQException
    {
        byte[] fm8Message;
        fm8Message =
                SimulationMessage.encode(federationName, "FM", "FS", "FM.8", ++this.messageCount, MessageStatus.NEW, "BGD1.1");
    ...
    public static void main(String[] args) throws Sim0MQException
    {
        ...
        new EMAFederationManager(federationName, fmPort, fsPort, localSk3);
